Attribute VB_Name = "No2TP_Ho"
Option Explicit
Dim tai As String
Dim taiA(84) As String ' tai Alphabet
Dim taiI(84) As Integer ' tai index number
...
Sub NumberToTpHolo()
    ...
    ' following the sequence of a, e, i, m, n, o, ou, u  (lower case, N, nn, upper case)
    ...
    If MsgBox("To convert Number Format to TP encoding?", vbYesNo) <> vbYes Then
        Exit Sub
    End If


    Application.Caption = "Converting Number Format to TP encoding. Please wait..."
    ...
    With c
        If toFindTone Then
            ...
            ElseIf (.Text >= "a" And .Text <= "z") Or (.Text >= "A" And .Text <= "Z") Then
                s = s + .Text
                .Text = ""
                ' check if ?nn, but not nn-
                If right(s, 2) = "nn" And Len(s) > 2 Then
                    s = left(s, Len(s) - 2) + ChrW$(176)
                End If
